...
export const fetchMovies: any = createAsyncThunk(
    'movie/fetchByTitle',
    async (search: string) => {
      const response: AxiosResponse<FetchMoviesResponse> = await movieAPI.fetchMovies(search)
...
export const fetchMoreMovies: any = createAsyncThunk(
    ...
    async (undefined, thunkApi) => {
      ...
      const response: AxiosResponse<FetchMoviesResponse> = await movieAPI.fetchMoreMovies(state.movies.currentSearch, state.movies.page+1)
      return response.data
    }
  )
